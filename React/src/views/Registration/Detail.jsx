  ...
  const content = (
    <div className="page">
        ...
        <div className="stagger" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px' }}>
              <DetailItem label="挂号编号" value={reg.regNo} delay={0} />
              <DetailItem label="患者" value={reg.patientName || user?.realName || user?.username} delay={1} />
              <DetailItem label="科室" value={reg.deptName} delay={2} />
              <DetailItem label="医生" value={reg.staffName} delay={3} />
              <DetailItem label="就诊时间" value={formatVisitSchedule(reg.workDate, reg.timePeriod)} delay={4} />
              <DetailItem label="挂号时间" value={formatDateTime(reg.regTime)} delay={5} />
              <DetailItem label="挂号费" value={formatMoney(reg.regFee)} delay={6} />
              <DetailItem label="挂号人" value={reg.registrantUserName || '—'} delay={7} />
            </div>
            {reg.status === 1 && (
      ...
      <ConfirmDialog
        ...
        title="退号确认"
        message={`确认退号 ${reg?.regNo}？号源将归还排班。`}
...
function DetailItem({ label, value, delay = 0 }) {
  return (
    <div className="view-reg-detail-item" style={{ animationDelay: `${delay * 60}ms` }}>
