    ...
    <PcLayout>
      ...
      {!loading && !error && (
        <div className="stagger" style={{ display: 'flex', gap: 16, marginBottom: 28, flexWrap: 'wrap' }}>
          <StatCard icon="📅" label="我的挂号" value={data?.registrations?.length ?? 0} color="#3d5a5c" index={0} />
          <StatCard icon="⏳" label="待缴费" value={data?.pendingCharges?.length ?? 0} color="#c8944a" index={1} />
          <StatCard icon="🏥" label="今日可挂号科室" value={data?.schedules?.length ?? 0} color="#7a9e85" index={2} />
          <StatCard icon="📋" label="就诊记录" value={data?.visits?.length ?? 0} color="#5a8590" index={3} />
        </div>
      )}
