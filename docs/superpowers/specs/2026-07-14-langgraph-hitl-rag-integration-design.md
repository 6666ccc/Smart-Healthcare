...
支持 `approve`、`edit`、`reject` 和 `respond`。服务端不信任客户端回传的工具
名称或允许决策集合；恢复时以 checkpoint 中的待确认操作为准校验请求。
...
操作，显示受控参数表单后提交。`respond` 使用文本输入补充信息。
...
- approve、edit、reject、respond 恢复流程；
