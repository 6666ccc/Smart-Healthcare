...
> 面向 HuiLiao 慧疗医院管理系统 — Python AI 服务侧  
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `ailearn_ai/memory/embeddings.py` | 新增 | `embed_query()`, `embed_documents()`, `_get_embedding_api_info()`, `_call_embedding_api()` |
| `ailearn_ai/memory/store.py` | 修改 | 删除 Embedding 相关函数，import 新模块 |
| `ailearn_ai/memory/retriever.py` | 新增 | `search_memories()`, `format_memories_for_prompt()`（从 store.py 迁移） |
| `ailearn_ai/memory/manager.py` | 新增 | 去重/TTL/摘要相关函数 |
| `ailearn_ai/memory/health.py` | 新增 | `check_memory_health()`, `_get_collection_info()` |
| `ailearn_ai/memory/__init__.py` | 修改 | 更新 import 路径 |

### P1-2：记忆去重

**目标**：写入前与已有记忆做相似度检查，> 0.95 则跳过。

**实现位置**：`ailearn_ai/memory/manager.py`
...
**注意**：`huiliao_user_profile` 和 `huiliao_knowledge` 两个 Collection **不设自动 TTL**，由业务逻辑显式管理生命周期。

### P1-4：双路并行检索

**目标**：单次查询同时检索记忆和用户画像，减少延迟。

**实现位置**：`ailearn_ai/memory/retriever.py`
...
### P2-1：新增 `huiliao_knowledge` Collection

**目标**：在 Qdrant 中创建知识库专用 Collection。

**实现位置**：`ailearn_ai/memory/knowledge.py`（新增）

```python
KNOWLEDGE_COLLECTION = "huiliao_knowledge"
...
```python
# ailearn_ai/memory/pipeline.py (新增)
...
**实现位置**：`ailearn_ai/memory/chunking.py`（新增）
...
| 文件 | 操作 | 说明 |
|------|------|------|
| `docs/SQL/huiliao.sql` | 修改 | 新增 `ai_knowledge_documents` 建表语句 |
...
```python
# ailearn_ai/API/routers/knowledge.py (新增)
...
**实现位置**：`ailearn_ai/memory/profile.py`（新增）
...
def apply_profile_updates(updates: list[ProfileUpdate]) -> None:
    """将抽取的更新写入 huiliao_user_profile collection。
