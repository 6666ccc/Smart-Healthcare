...
│              │                        │                          │
│              │  Collection 1:          │                          │
│              │   huiliao_chat_memory   │  会话记忆                 │
│              │                        │                          │
│              │  Collection 2:          │                          │
│              │   huiliao_user_profile  │  用户画像                 │
│              │                        │                          │
│              │  Collection 3:          │                          │
│              │   huiliao_knowledge     │  知识库文档               │
│              └────────────────────────┘                          │
└──────────────────────────────────────────────────────────────────┘
```
...
当前 [ailearn_ai/memory/store.py](../ailearn_ai/memory/store.py#L136-L185) 的 `add_memory()` 已存储 `conversation_id`、`user_id`、`role`、`content`、`created_at` 五个字段。需要扩展 Point payload 以支持上述元数据标签。
...
代码位置：[ailearn_ai/chains/qa.py](../ailearn_ai/chains/qa.py#L163-L166)  
代码位置：[ailearn_ai/memory/store.py](../ailearn_ai/memory/store.py#L251-L311)
...
| 父子切片 | ❌ 未实现 | 按文档类型差异化切分 |
| 知识库 RAG | ❌ 未实现 | 新增 huiliao_knowledge collection |
| 用户画像 | ❌ 未实现 | 新增 huiliao_user_profile collection |
