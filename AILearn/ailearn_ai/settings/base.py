...
def get_huiliao_api_base_url() -> str:
    return (os.getenv("HUILIAO_API_BASE_URL") or "http://localhost:8080").strip().rstrip("/")


def get_huiliao_api_token() -> str:
    """已废弃：改用 get_huiliao_api_key()。保留以兼容旧配置。"""
    return (os.getenv("HUILIAO_API_TOKEN") or os.getenv("HUILIAO_TOKEN") or "").strip()


def get_huiliao_api_key() -> str:
    """内部 API Key，用于 AI 服务回调 Java 时的服务间鉴权（X-Api-Key 头）。

    优先级：HUILIAO_API_KEY > HUILIAO_API_TOKEN（旧兼容）> HUILIAO_TOKEN（旧兼容）。
    """
    return (
        os.getenv("HUILIAO_API_KEY")
        or os.getenv("HUILIAO_API_TOKEN")
        or os.getenv("HUILIAO_TOKEN")
        or ""
    ).strip()


def get_huiliao_auth_headers() -> dict[str, str]:
    """构建回调 Java 的请求头：X-Api-Key（服务间鉴权）+ X-User-Id（用户上下文）。

    优先使用当前请求上下文（ContextVar），否则回退 .env 配置。
    """
    from ailearn_ai.auth_context import get_api_key, get_user_id

    headers = {"Content-Type": "application/json", "Accept": "application/json"}

    api_key = get_api_key() or get_huiliao_api_key()
    if api_key:
