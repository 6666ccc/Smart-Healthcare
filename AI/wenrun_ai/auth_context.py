...
_request_api_key: ContextVar[str | None] = ContextVar("wenrun_api_key", default=None)
_request_user_id: ContextVar[int | None] = ContextVar("wenrun_user_id", default=None)
...
@contextmanager
def wenrun_api_context(api_key: str | None, user_id: int | None = None):
    """在 Agent 执行期间绑定 api_key 与 user_id，结束后自动恢复。

    Tools 回调 Java 时通过 get_wenrun_auth_headers() 读取这些值，
    设置 X-Api-Key 与 X-User-Id 请求头。
    """
    from wenrun_ai.settings import base

    key = api_key or base.get_wenrun_api_key()
